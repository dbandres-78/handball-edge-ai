  ...
  return (
    <div className="max-w-4xl mx-auto px-5 py-10">
      ...
      <p style={{ fontSize: 13, color: C.muted, marginBottom: 12 }}>
        ...
        También puedes crear un club y su plantilla directamente, sin esperar a dar de alta un partido.
      </p>
      ...
      {clubs.length === 0 ? (
        <div className="text-center py-12 rounded-lg" style={{ color: C.faint, fontSize: 13, border: `1px dashed ${C.line}` }}>
          Aún no hay clubes en el catálogo. Créalo arriba, o se crean también al dar de alta partidos con plantillas.
        </div>
      ) : (
          ...
          {clubs.map((c) => (
            <Link key={c.id} href={`/clubs/${c.id}`} className="flex items-center gap-3 p-3 rounded-lg"
              ...
              {c.photoUrl ? (
                ...
                <img src={c.photoUrl} alt={c.name} className="w-9 h-9 rounded-md object-cover flex-shrink-0" style={{ border: `1px solid ${C.line}` }} />
              ) : (
