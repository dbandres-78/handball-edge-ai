...
function rowToUiEvent(r: any): UiEvent {
  ...
  // El dorsal que sale se guarda como playerOutId ("SIDE:dorsal"); lo devolvemos como número.
  const subOut = payload.playerOutId ? Number(payload.playerOutId.split(':')[1]) : null;
  return {
    id: r.seq,
    ...
    subOutNumber: Number.isFinite(subOut) ? subOut : null,
  };
}
...
export function makePgMatchesRepository(db: Queryable): PgMatchesRepository {
      ...
      await db.query(
        ...
        [matchId, seq++, Math.round(e.t * 1000), e.period, e.side, e.playerNumber ?? null, e.type,
         JSON.stringify({
           ...
           // SUBSTITUTION: quién entra (playerNumber) y quién sale (subOutNumber), como ids canónicos.
           playerInId: e.type === 'SUBSTITUTION' && e.playerNumber != null ? `${e.side}:${e.playerNumber}` : null,
           playerOutId: e.type === 'SUBSTITUTION' && e.subOutNumber != null ? `${e.side}:${e.subOutNumber}` : null,
         })],
      );
    }
  }
  ...
  const repo: PgMatchesRepository = {
    ...
    async delete(matchId) {
      ...
      await db.query('DELETE FROM match_event     WHERE match_id=$1', [matchId]);
      await db.query('DELETE FROM player          WHERE match_id=$1', [matchId]);
      await db.query('DELETE FROM team            WHERE match_id=$1', [matchId]);
      await db.query('DELETE FROM match_read_model WHERE match_id=$1', [matchId]);
      await db.query('DELETE FROM match           WHERE id=$1', [matchId]);
    },
  };
