...
export function buildRoster(home: UiTeam, away: UiTeam, matchId = 'live'): ResolvedRoster {
  ...
  ([['HOME', home], ['AWAY', away]] as const).forEach(([side, team]) => {
    // Si el equipo tiene alguna titularidad marcada, se respeta la alineación (base del ± fino).
    // Si no, todos se consideran en pista (degradación honesta: el ± cae al diferencial de equipo).
    const lineupDefined = team.players.some((p) => p.starter);
    for (const p of team.players) {
      players.push({
        playerId: `${side}:${p.number}`, teamId: teamId(side),
        side, number: p.number, name: p.name,
        position: p.gk ? 'GK' : 'NA', starter: lineupDefined ? !!p.starter : true,
      });
    }
  });
  ...
  return sorted.map((e, seq) => {
    ...
    if (e.type === EventType.SHOT && e.outcome) {
      ...
      payload = {
        ...
        playerInId: e.playerNumber != null ? `${e.side}:${e.playerNumber}` : null,
      };
    }
    return {
      matchId, seq,
...
/**
 * Jugadores en pista de un equipo en el instante `atTime`, derivado igual que en el core:
 * titulares (o todos, si no hay alineación) + SUBSTITUTION + GOALKEEPER_CHANGE hasta ese momento.
 * La UI lo usa para pintar pista/banquillo y validar cambios; el ± lo calcula el core desde eventos.
 */
export function onCourtAt(team: UiTeam, events: UiEvent[], side: Side, atTime: number): Set<number> {
  const lineupDefined = team.players.some((p) => p.starter);
  const set = new Set<number>();
  for (const p of team.players) if (lineupDefined ? p.starter : true) set.add(p.number);
  let gk = team.players.find((p) => p.gk && (lineupDefined ? p.starter : true))?.number;

  const relevant = events
    .filter((e) => e.side === side && e.t <= atTime && (e.type === EventType.SUBSTITUTION || e.type === EventType.GOALKEEPER_CHANGE))
    .sort((a, b) => a.t - b.t || a.id - b.id);
  for (const e of relevant) {
    if (e.type === EventType.SUBSTITUTION) {
      if (e.subOutNumber != null) set.delete(e.subOutNumber);
      if (e.playerNumber != null) set.add(e.playerNumber);
    } else if (e.playerNumber != null) {   // GOALKEEPER_CHANGE: portero por portero
      if (gk != null && gk !== e.playerNumber) set.delete(gk);
      set.add(e.playerNumber);
      gk = e.playerNumber;
    }
  }
  return set;
